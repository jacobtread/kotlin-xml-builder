 ...
 */
inline fun xml(root: String, encoding: String? = null, version: XmlVersion? = null, init: Node.() -> Unit): Node {
    val node = xml(root, encoding, version)
    node.init()
    return node
}
 ...
 * @return The created Node
 */
fun xml(root: String, encoding: String? = null, version: XmlVersion? = null): Node {
	val node = Node(root)
	if (encoding != null) {
